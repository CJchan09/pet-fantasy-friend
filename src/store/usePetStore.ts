...
import { getLifecycleStatus } from '@/domain/petLifecycle'
...
export function usePetStore() {
  ...
  const lastGrowthAt = useGameStore((s) => s.state.lastGrowthAt)
  const feedPet = useGameStore((s) => s.feedPet)

  return {
    pet,
    lifecycleStatus: getLifecycleStatus(lastGrowthAt),
